...
from src.database.tables.geek_transactions import (
...
def fetch_letest_transaction(db_client: DatabaseClient) -> tuple[int, int]:
    ...
    logger.debug(f"最新のトランザクションを取得しました: 最新のブロック番号: {latest_block_number}, 最新のインデックス: {latest_log_index}")
    return latest_block_number, latest_log_index
    
   
def update_geek_transactions(db_client: DatabaseClient):
    logger.info("geek_transactionsを開始します")
    ...
    logger.info("geek_transactionsを終了します")
