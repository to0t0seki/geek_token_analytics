...
def create_geek_transactions(db_client: DatabaseClient) -> None:
    create_table_query = """
    CREATE TABLE IF NOT EXISTS geek_transactions_oas (
    ...
    CREATE INDEX IF NOT EXISTS idx_gt_oas_timestamp ON geek_transactions_oas(timestamp);
    """
    db_client.execute(create_index_timestamp_query)
        
    create_index_from_address_query = """
    CREATE INDEX IF NOT EXISTS idx_gt_oas_from_address ON geek_transactions_oas(from_address);
    """
    db_client.execute(create_index_from_address_query)

    
    create_index_to_address_query = """
    CREATE INDEX IF NOT EXISTS idx_gt_oas_to_address ON geek_transactions_oas(to_address);
    """
...
def insert_geek_transactions(db_client: DatabaseClient, transactions: list) -> int:
    ...
    INSERT INTO geek_transactions_oas (block_number, log_index, tx_hash, timestamp, from_address, to_address, value, method, type)
