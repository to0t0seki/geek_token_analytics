...
def create_other_wallets(db_client: DatabaseClient) -> None:
    params = {
    ...
    create_query = """
    CREATE MATERIALIZED VIEW IF NOT EXISTS other_wallets AS
    ...
    create_index_query = """
    CREATE UNIQUE INDEX IF NOT EXISTS idx_other_wallets_address 
    ON other_wallets(address);
    """
    db_client.execute(create_index_query)


def refresh_other_wallets(db_client: DatabaseClient) -> None:
    refresh_query = """
    REFRESH MATERIALIZED VIEW CONCURRENTLY other_wallets;
    """
    db_client.execute(refresh_query)

client = DatabaseClient()
create_other_wallets(client)
